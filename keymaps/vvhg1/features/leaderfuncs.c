...
void *leader_start_func(uint16_t keycode) {
    switch (keycode) {
        ...
        //     break;
        case KC_D:
            return camel_case_or_open_download;
#endif
        case KC_LSFT:
        case KC_RSFT:
        case ML_sft:
...
void *camel_case_or_open_download(uint16_t keycode) {
    switch (keycode) {
        case KC_R:
            enable_xcase_with(OSM(MOD_LSFT));
            return NULL;
        case KC_L:
            tap_code16(LGUI(KC_7)); //  open recent download
            return NULL;
        default:
            return NULL;
    }
...
void *umlaut_stuff(uint16_t keycode) {
    switch (keycode) {
        case KC_S:
            ...
            if (is_mac) {
                // mac unicode input (using option key)
