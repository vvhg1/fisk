...
// MA: Set marker for selection
// MO: Select from cursor to marker
// MM: Move to marker
...
// Q: Close tab
// R: Start debugging
// R shifted: Run without debugging
// T: Open terminal
...
// VMO: Move tab to the right
// VFN: Focus tab to the left
// VFO: Focus tab to the right
// VP:  Format selection
// VAP: Format document
...
void *leader_start_func(uint16_t keycode) {
...
#else
  case KC_M:
    return vscode_markers; // here m is the start for setting markers
#endif
  case KC_N:
  ...
  case KC_R:
    if (get_mods() &
        MOD_MASK_SHIFT) { // run w/o debugging, with shift stop debugging
      tap_code(KC_F5);
    } else {
      tap_code16(C(KC_F5));
    }
...
void *vscode_markers(uint16_t keycode) {
  ...
  case KC_A:
    // set anchor
    tap_code16(C(KC_K)); //  vg
    tap_code16(C(KC_B)); //  vg
    return NULL;
  case KC_O:
    // select to anchor
    tap_code16(C(KC_K)); //  vg
...
void *operator_stuff(uint16_t keycode) {
  switch (keycode) {
  case KC_G: // larger or larger equal
    tap_code16(S(KC_DOT));
    tap_code16(KC_EQL);
    return NULL;
...
void *umlaut_stuff(uint16_t keycode) {
  ...
  case KC_S:
    ...
    if (is_windows) {
      // windows
      SEND_STRING(SS_LALT(SS_TAP(X_P0) SS_TAP(X_P2) SS_TAP(X_P2)
                              SS_TAP(X_P3))); // here S ß
    } else {
      // linux
      tap_code16(C(S(KC_U)));
  ...
  case KC_U:
    if (get_mods() & MOD_MASK_SHIFT) {
      ...
      if (is_windows) {
        // windows
        SEND_STRING(
            SS_LALT(SS_TAP(X_P1) SS_TAP(X_P2) SS_TAP(X_P9))); //  here U Ü
      } else {
        // linux
        tap_code16(C(S(KC_U)));
  ...
  case KC_O:
    if (get_mods() & MOD_MASK_SHIFT) {
      ...
      if (is_windows) {
        // windows
        SEND_STRING(SS_LALT(SS_TAP(X_P0) SS_TAP(X_P2) SS_TAP(X_P1)
                                SS_TAP(X_P4))); // here O Ö
      } else {
        // linux
        tap_code16(C(S(KC_U)));
      ...
      if (is_windows) {
        // windows
        SEND_STRING(
            SS_LALT(SS_TAP(X_P1) SS_TAP(X_P4) SS_TAP(X_P8))); // here O Ö
      } else {
        // linux
        tap_code16(C(S(KC_U)));
...
void *vscode_stuff_brackets(uint16_t keycode) {
  ...
  case KC_D:
    select_in_bracket();
    tap_code16(C(KC_X)); //  vg
    return NULL;
  case KC_P:
    select_in_bracket();
    tap_code16(C(KC_V)); //  vs
    return NULL;
  case KC_Y:
    select_in_bracket();
    tap_code16(C(KC_C)); //  vs
    wait_ms(100);
    tap_code16(KC_ESC); //  vg
    return NULL;
  default:
    return NULL;
  }
...
/* #ifdef ENCODER_ENABLE
void *encoder_stuff(uint16_t keycode) {  // e+  h horizontal scroll / v vertical
scroll / e editor / t tab / p paging switch (keycode) { case KC_H:
